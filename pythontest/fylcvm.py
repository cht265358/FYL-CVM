...
class FYLCVM:       #base class for FCC
    ...
    def compute_basic_cluster_prob(self,partition,site_potential):
        spm=np.append(np.zeros(self.clustersize),site_potential)
    ...
    def compute_total_energy(self,site_potential,prob,T):
        ...
        for i in range(self.component):
            probi=0
            for j,k,l in itertools.product(range(self.component), repeat=3):
                probi+=prob[i][j][k][l]
            print(probi)
            pointenergy+=probi*np.log(probi)*T
        for j in range(self.component):
            probi=0
            for i,k,l in itertools.product(range(self.component), repeat=3):
                probi+=prob[i][j][k][l]
            print(probi)
            pointenergy+=probi*np.log(probi)*T
        for k in range(self.component):
            probi=0
            for i,j,l in itertools.product(range(self.component), repeat=3):
                probi+=prob[i][j][k][l]
            print(probi)
            pointenergy+=probi*np.log(probi)*T
        for l in range(self.component):
            probi=0
